...
reinit:
		...
		if (!this->vHandle)
		{
			...
			{
				memMap = false;
				LOG("[!] 内存映射初始化失败，请尝试不使用MMap\n");
				goto reinit;
			}
			LOG("[!] 初始化失败!DMA正在使用或者断开连接\n");
			return false;
		}
...
size_t Memory::GetBaseDaddy(std::string module_name)
{
	...
	{
		LOG("[!] 获取Base Address失败 %s\n", module_name.c_str());
		return 0;
	}

	LOG("[+] 获取Base Address成功 %s  0x%p\n", module_name.c_str(), module_info->vaBase);
...
bool Memory::FixCr3()
{
	...
	{
		LOG("[-] VMMDLL_InitializePlugins调用失败\n");
		return false;
	}
...
{
	if (!VMMDLL_MemRead(this->vHandle, pid, address, (PBYTE)buffer, size))
	{
		LOG("[!] 读取内存失败 0x%p\n", address);
		return false;
	}
...
{
	static bool logOnce = true; // 静态变量，用于确保日志只输出一次
	...
	{
		if (logOnce) // 只在第一次失败时输出日志
		{
			LOG("[-] 执行分散读取失败(如果是载入游戏时出现请无视)\n");
			logOnce = false; // 设置为false，防止再次输出
		}
	}
