...
struct LocalPlayer {
    ...
    void BigMapRADAR()
    {
        uint64_t Local_Team = Team;
        if (mem.IsValidPointer(Local_Team))
        {
            for (int i = 0; i <= 2000000; i++)
            {
                int SpeTeam = 1;
                auto handle = mem.CreateScatterHandle();
                mem.AddScatterWriteRequest(handle, Team, &SpeTeam, sizeof(SpeTeam));
            ...
            {
                auto handle = mem.CreateScatterHandle();
                mem.AddScatterWriteRequest(handle, Team, &Local_Team, sizeof(Local_Team));
                mem.ExecuteWriteScatter(handle);
                mem.CloseScatterHandle(handle);
            }
        }
