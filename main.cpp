...
void MiscBaseScatter(Level* map, LocalPlayer* myself, Camera* gameCamera, Sense* esp) {
    ...
    //武器上色
    /*uint64_t ViewModelHandleAddress = mem.OFF_BASE + OFF_LOCAL_PLAYER + OFFSET_VIEWMODEL;
    ViewModelHandleAddress = ViewModelHandleAddress & 0xFFFF;
    uint64_t viewmodelPointerAddress = mem.OFF_BASE + OFF_ENTITY_LIST + (ViewModelHandleAddress * 0x20);
    mem.AddScatterReadRequest(handle, viewmodelPointerAddress, &esp->ViewModelPtr, sizeof(uint64_t));
    */
    ...
    for (size_t i = 0; i < players.size(); ++i) {
        ...
        if (mem.IsValidPointer(player->BasePointer)) {
            ...
            if (player->IsDummy() && Map->IsFiringRange || player->IsPlayer()) {
                ...
                ESP->playeruid = player->BasePointer + OFFSET_UID;
                mem.AddScatterReadRequest(handle, ESP->playeruid, &player->UidPointer, sizeof(uint64_t));
            }
...
int main()
{
	...
	{
		std::cout << "[#] 初始化 DMA 失败" << std::endl;
        std::cout << "[#] 按 ENTER 继续...";
        std::cin.get();
	}
    std::cout << "[#] DMA 初始化完成" << std::endl;

    if (!mem.GetKeyboard()->InitKeyboard())
    {
        std::cout << "[#] 通过内核初始化键盘热键失败" << std::endl;
        std::cout << "[#] 按 ENTER 继续...";
        std::cin.get();
    }
